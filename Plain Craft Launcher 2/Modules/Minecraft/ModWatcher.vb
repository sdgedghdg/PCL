Public Module ModWatcher
    ...
    Private _HasRunningMinecraft As Boolean = False
    Public Property HasRunningMinecraft As Boolean
        Get
            Return _HasRunningMinecraft OrElse McTrackedInstances.Any(Function(t) Not t.Process.HasExited)
        End Get
        Set(value As Boolean)
            _HasRunningMinecraft = value
        End Set
    End Property

    '进程追踪（重启后可识别旧进程）
    Public McLaunchInstanceId As String = ""
    Private TrackingFilePath As String = PathExeFolder & "PCL\ProcessTracking.json"
    Private McTrackedInstances As New List(Of TrackedInstance)
    Private TrackedMonitorRunning As Boolean = False
    Private Class TrackedInstance
        Public Id As String
        Public Process As Process
        Public StartTime As Date
        Public Sub New(id As String, proc As Process, startTime As Date)
            Me.Id = id
            Me.Process = proc
        ...
        Public Sub Kill()
            ...
            Catch ex As Exception
                Logger.Error(ex, "强制结束追踪的 Minecraft 进程失败", LogBehavior.Toast)
            End Try
        End Sub
    End Class
    ...
    Private Sub SaveTracking()
        Try
            ...
            For Each t In McTrackedInstances
                data.Add(New JObject From {
                    ...
                    New JProperty("startTime", t.StartTime.ToString("o")),
        ...
        End Try
    End Sub
    Private Sub RemoveTracking(id As String)
        McTrackedInstances.RemoveAll(Function(t) t.Id = id)
        SaveTracking()
    ...
    Public Sub LoadTrackedProcesses()
        ...
        End Try
    End Sub
    Private Sub EnsureTrackedMonitorRunning()
        If TrackedMonitorRunning Then Return
        TrackedMonitorRunning = True
        RunInNewThread(
        Sub()
            Try
                Do
                    Thread.Sleep(5000)
                    Dim deadIds As New List(Of String)
                    SyncLock McTrackedInstances
                        For Each t In McTrackedInstances.ToList()
                            If t.Process.HasExited Then
                                deadIds.Add(t.Id)
                                McTrackedInstances.Remove(t)
                            End If
                        Next
                ...
                Loop
            Catch ex As Exception
                Logger.Error(ex, "追踪的 Minecraft 进程监控出错")
                TrackedMonitorRunning = False
            End Try
        End Sub, "Tracked Process Monitor")
    End Sub
